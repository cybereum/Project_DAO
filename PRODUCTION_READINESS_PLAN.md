...
|---|-----------|:-----:|:----:|----------|----------|
| 1 | **Smart Contract Security** | **8** | — | Custom reentrancy guard on all ETH paths. SafeERC20 on all 9 ERC-20 call sites. `whenNotPaused` on every state-changing function. TimelockLib (24h delay) for treasury/fee changes. `MIN_FEE_BPS = 1` enforced. Repo also includes `AUDIT_REPORT.md`, which is an AI-assisted review rather than a formal external audit; its dated findings were considered in this assessment and are treated as remediated and/or outdated unless revalidated against current code. | No formal third-party audit yet. Contract handles real value, so an independent external audit remains the #1 blocker before production. |
...
| 3 | **Deployability** | **7** | — | Library-linked architecture keeps bytecode under 24KB. Deploy scripts exist (`deploy.js`, `deploy-split.js`) with balance validation and network detection. Split-contract deployment path is present and covered by dedicated split tests. | `sdk/deployments.json` has placeholder addresses ("pending-deployment"). Production/testnet addresses still need to be written back to the deployment registry after deployment. No staging environment. |
