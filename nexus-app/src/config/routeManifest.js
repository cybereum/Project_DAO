export const BASE_URL = 'https://www.cybereum.io';
...
export const ROUTE_MANIFEST = [
  ...
  },
  {
    path: '/ngo',
    isPublic: true,
    sitemap: { changefreq: 'weekly', priority: '0.88' },
    seo: {
...
export const ROUTE_SEO = Object.fromEntries(
  ROUTE_MANIFEST
    .filter((route) => route.seo)
    .map((route) => [route.path, route.seo]),
);

export const PUBLIC_ROUTES = ROUTE_MANIFEST
  .filter((route) => route.isPublic)
  .map((route) => route.path);

export const PERSONA_ROUTES = ['/agents', '/builders', '/ngo', '/enterprise', '/cities'];

export const SITEMAP_ROUTES = ROUTE_MANIFEST.filter((route) => route.sitemap);
